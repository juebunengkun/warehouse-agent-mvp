...
def generate_dqc(state: AgentState) -> AgentState:
    ...
    metric_mismatch = " OR\n  ".join(
        [f"COALESCE(d.{field}, 0) <> COALESCE(a.{field}, 0)" for field in metric_fields]
    )

    rows = [
        ["分区完整性", names["dwd"], "当日分区记录数 > 0", "阻断"],
        ["主键/粒度唯一", names["dws"], f"{key_expr} 在同一 dt 下唯一", "阻断"],
        ["维度非空", names["dws"], f"{key_expr} 不为空", "阻断"],
        ["指标非负", names["dws"], "金额、次数、人数类指标 >= 0", "阻断"],
        ["波动监控", names["ads"], "核心指标日环比波动超过 50% 告警", "告警"],
        ["层间一致", f"{names['dws']} vs {names['ads']}", "ADS 应按同一粒度与 DWS 指标一致", "阻断"],
    ]

    metric_checks = "\n".join(
        [
            f"SELECT COUNT(1) AS bad_rows FROM {names['dws']} WHERE dt='${{bizdate}}' AND {field} < 0;"
